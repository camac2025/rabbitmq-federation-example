...
#Publish several messages to upstream - all after the first will end up on both upstream and downstream
printf "\npublish\n---------------------------------------\n"
curl -i -X POST -H "Content-Type: text/plain" "$HTTPS_UPSTREAM/api/exchanges/$VHOST/${EXCHANGE}/publish" \
      -d '{"properties":{},"routing_key":"'${RKEY}'","payload":"message 1","payload_encoding":"string"}'

curl -i -X POST -H "Content-Type: text/plain" "$HTTPS_UPSTREAM/api/exchanges/$VHOST/${EXCHANGE}/publish" \
      -d '{"properties":{},"routing_key":"'${RKEY}'","payload":"message 2","payload_encoding":"string"}'

curl -i -X POST -H "Content-Type: text/plain" "$HTTPS_UPSTREAM/api/exchanges/$VHOST/${EXCHANGE}/publish" \
      -d '{"properties":{},"routing_key":"'${RKEY}'","payload":"message 3","payload_encoding":"string"}'
